  ...
  if (withLoader) {
    ...
    if (loader) {
      ...
      if (video) {
        ...
        // Fallback: reveal options after 30 s even if video stalls forever.
        const fallbackTimer = setTimeout(safeReveal, 30000);
        const clearFallback = () => clearTimeout(fallbackTimer);

        video.addEventListener("ended", () => { clearFallback(); safeReveal(); }, { once: true });
        video.addEventListener("error", () => { clearFallback(); safeReveal(); }, { once: true });

        // Set src directly on the element (not via <source> child) so the
        // error event reliably fires on the video element when the file is missing.
        video.src = loader._loaderSrc;
        video.load();

        // Try to play with sound first. If blocked, fall back to muted autoplay.
        video.muted = false;
        video.play().catch(() => {
          video.muted = true;
          video.play().catch(() => {
